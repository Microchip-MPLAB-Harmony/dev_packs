...
def resetExtension():
    ...
    # 1 msec with reset and clock low
    dev.Pins(0, dev.RESET | dev.SWCLK, timeout)
...
def ExitResetExtensionL10(brext):
    ...
    if reply != 0:
        log.error("User page validation failure with error code %08x" % reply)
...
def AddBootHashLS(idauIdauProt,idauBnscSize,pageCopy,bootopt):
    ...
    if bootopt == 1 or bootopt == 2 or bootopt  > 4: # SHA256 or SHA256BOOTKEY    
        if bootopt == 2:
        ...
        if idauBnscSize > 0:
            utils.sha256_update(content, idauBsSize - idauBnscSize , idauBnscSize)
        #TODO iComputeRawSignatureFromHash(content, idauBsSize - idauBnscSize) # compute signature inplace
        
    # Write Hash at proper location
    program_row(rowStartAddr,rowStartAddr,content)

def AddBootHashL11(idauBsSize,idauBnscSize,pageCopy,bootopt):
    utils.sha256_reset()
    if bootopt != 1:
        utils.sha256_update(pageCopy, FUSES_BOOTROM_BOOTKEY,FUSES_BOOTROM_BOOTKEYSIZE)
        utils.sha256_update(pageCopy, FUSES_BOOTROM_BOOTKEY,FUSES_BOOTROM_BOOTKEYSIZE)
...
def hmac_sha256(ptrkey,  ofsKey, keySizeInBytes, ptrmsg, ofsMsg, msgSizeInBytes, hash_in_out, out_ofs):
    # inner : H(key + message))
	# assumes utils.sha256_reset() was already called
...
def write_bocor_ls(pageCopy):
    ...
    if "LS" in device:
        bootlck = getFuse(pageCopy, 51, 51) 
        if bootlck == 0:
    ...
    bootprot = getFuse16(pageCopy, 40, 50) * BP_GRANULARITY
    bootopt = getFuse(pageCopy, 32, 39)
    bnsc = getFuse16(pageCopy, 19, 27)*BNSC_GRANULARITY
    ...
    if (bootprot >= 0):
        AddBootHashLS(bootprot, bnsc, pageCopy, bootopt)
    else:
...
def write_cfg_page(address, data, ofs):
    ...
    for i in range(0, 256):
        if pageCopy[i] != data[ofs+i]:
            if address == UROW and (i< USERCRC or i>USERCRC+3):
                log.info("Application user page  byte at 0x%x differs from default : from 0x%02x to 0x%02x" % (address+i, pageCopy[i], data[ofs+i]))
    ...
    if (boot_prot >= bs and bs-bnsc >= 0x100):
        AddBootHashL11(bs, bnsc, pageCopy,sec_boot)
    else:
        raise Exception("BOOTPROT region too small or BNSC too small")
