...
def onShowPropsDFP(t):
    ...
    if "LS60" in device:
        ide.addTextProperty(t,"x.prk.path" , "*|programoptions|programoptions.eraseb4program","Private EC key for signing bootloader","Enter path to private key used to sign bootloader. File must be unencrypted (i.e. use <openssl pkcs8 -topk8 -inform pem -in in.pem -outform pem -nocrypt -out out.pem>)", "")
        ide.addTextProperty(t,"x.puk.path" , "*|programoptions|programoptions.eraseb4program","Public EC key for provisioning the ECC","Enter path to public key to provision the ECC608.",  "")
