...
def prog_write(type_of_mem, address, length, data):
  ...
  dev.WriteMem(address, data, 0, length)
...
def prog_read(type_of_mem, address, length, data):
  ...
  dev.ReadMem(address, data, 0, length)
...
def debug_read(mem_type, start, length, data):
  ...
  res = dev.ReadMem(start, data, 0, length)
  if res == 1:
...
def debug_write(mem_type, start, length, data):
  ...
  res = dev.WriteMem(start, data, 0, length)
  if res < 0:
...
def send_customer_bp_info(info):
  if info.bpAction == action.set:
    if info.bpType == b_type.program:
      ...
      dev.SetBPEx(info.address,  -16L | m )
    else:
      log.info("Setting DATA")
  else:
    log.info("Clearing breakpoint")
    dev.ClearBPEx(-1)
    dev.ClearDataEvent(-1)
