...
def is_nvm_ready():
    sts = dev.Read16(nvm.stsReg)
    isReady = 0 != (sts & 1)
    if not isReady:
        return False

    intFlag = dev.Read16(nvm.intReg)
    isError = 0 != (intFlag & (2 | 4 | 8 | 16 | 32 | 64)) # Check ERROR flags
    if not isError:
        return True
    # clear error bits
    log.error("NVMCtrl error bits set! (STATUS = 0x%X, INTFLAG = 0x%X)" % (sts, intFlag))
    dev.Write16(nvm.intReg, intFlag | (2 | 4 | 8 | 16 | 32 | 64)) 
...
def set_autowrite():
    global prevCfgReg
    prevCfgReg = dev.Read16(nvm.cfgReg)
    newCfgReg = (prevCfgReg & 0xffc8) | 0x0034;  # set autows and auto page write
    if newCfgReg != prevCfgReg:
        dev.Write16(nvm.cfgReg, newCfgReg)

def restore_cfg():
    #log.debug("reset_cfg_word: prevCfgReg = %x" % prevCfgReg)
    dev.Write16(nvm.cfgReg, prevCfgReg)
