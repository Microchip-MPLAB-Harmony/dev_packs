...
def begin_communication_session():
    ...
    log.info("Cortex-M0+ NVMCTRL programmer/debugger")
...
def clearNvmErrorBits():
    ...
    dap.WriteD16(flash_info.baseAddress + flash_info.stsRegOfs, status | 0x001E) # clear error bits and load bit by setting 1
    dap.WriteD8(flash_info.baseAddress + flash_info.intFlagOfs, intFlag | 2) # clear error bit by setting 1
...
def eraseRow(addr):
    ...
    status = dap.ReadD16(flash_info.baseAddress + flash_info.stsRegOfs)
    intFlag = dap.ReadD8(flash_info.baseAddress + flash_info.intFlagOfs)
    log.info("eraseRow: NVMCtrl STATUS = 0x%X, INTFLAG = 0x%X" % (status, intFlag))
...
def writeCommand(command, t):
    dap.WriteD16(flash_info.baseAddress + flash_info.cmdRegOfs, command)
    while t > 0 :
        if isNvmReady():
            return
        time.sleep(0.001)
...
def write_flash_page(adr, ofs, data, total_length):
    ...
    dap.WriteD16(flash_info.baseAddress + flash_info.cmdRegOfs, CMD_WRITE_PAGE)
...
def prog_write(type_of_mem, address, length, data):
    ...
    if address == NVMCTRL_USER: # user page / fuses / configuration bits
        if length > 256:
            log.error("prog_write, Cfg, length (%X) > 0x100 bytes, only handle writing 0x100 bytes for user page!" % length)
