...
def _step_target():
    #TODO : We should probably move this to a gen4 script
    global g_is_running
    reg0 = read_named_register("R0")
    # get current pc
    pc = _get_pc()
    # store current bkpt register value for BVR0
    bkpt_value0 = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR0)
    # store current bkpt control register value for BCR0
    bkpt_ctrl0 = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR0)
    # store current bkpt register value for BVR1
    bkpt_value1 = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR1)
    # store current bkpt control register value for BCR1
    bkpt_ctrl1 = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR1)
    # store current bkpt register value for BVR2
    bkpt_value2 = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR2)
    # store current bkpt control register value for BCR2
    bkpt_ctrl2 = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR2)
    # Disable all other hw bps for now, we are just using BCR0
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR1,0x0)
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR2,0x0)
    pc_bkpt = (pc & ARMv7A_DBGBVRx_ADDR_MASK)
    # Set the breakpoint
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR0,pc_bkpt)
    # Enable the breakpoint, with address mismatch
    # Basically we write the PC address that's going to execute, which will trigger an 
    # Address mismatch halt
    if is_thumb_mode():
        if pc % 4 == 0: # Word aligned
            dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR0, ARMv7A_DBGBCR_M_IVA_MISMATCH | ARMv7A_DBGBCR_C_BYTE_ACCESS_2BYTES_0 | ARMv7A_DBGBCR_C_SUPER_ANY | ARMv7A_DBGBCR_C_BKPT_ENABLED)
        else:
            dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR0, ARMv7A_DBGBCR_M_IVA_MISMATCH | ARMv7A_DBGBCR_C_BYTE_ACCESS_2BYTES_2 | ARMv7A_DBGBCR_C_SUPER_ANY | ARMv7A_DBGBCR_C_BKPT_ENABLED)
    else:
        dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR0, ARMv7A_DBGBCR_M_IVA_MISMATCH | ARMv7A_DBGBCR_C_BYTE_ACCESS_4BYTES | ARMv7A_DBGBCR_C_SUPER_ANY | ARMv7A_DBGBCR_C_BKPT_ENABLED)
    # Restore R0, before we run
    write_named_register("R0",reg0)
    # Enable Halting debug-mode enable
    enable_halting_debug()
    dbgdscr = dev.Read32(ARMv7A_DEBUG_BASE+ARMv7A_DBGDSCR)
    log_dbgdscr(dbgdscr)
    # Run target, it should halt in the next instruction, because of an address mismatch
    dev.runScript("Run")
    for i in range(10):
        if not _is_target_running():
            break
        dev.Delay(100)
    if g_is_running:
        log.error("Debug: Target did not halt on step")
    
    # Enable Halting debug-mode enable
    enable_halting_debug()
    # Restore breakpoint register value and breakpoint control register value
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR0,(bkpt_value0))
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR0,bkpt_ctrl0)
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR1,(bkpt_value1))
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR1,bkpt_ctrl1)
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBVR2,(bkpt_value2))
    dev.Write32(ARMv7A_DEBUG_BASE+ARMv7A_DBGBCR2,bkpt_ctrl2)
