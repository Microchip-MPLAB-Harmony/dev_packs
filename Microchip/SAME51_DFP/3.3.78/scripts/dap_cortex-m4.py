...
def cache_invalidate():
    dev.Write32(nvm.cfgReg + 0x2000 + 0x20,1)
def cache_onDebugStart():
    cache_invalidate()
def cache_onStop():
    cache_invalidate()
def cache_onDataRead(start,length):
    cache_invalidate()
def cache_onDataWrite(start,length):
    cache_invalidate()
...
def erase():#mplab
    ...
    if settings.getBool("restore.bootprot.nvmctrlregionlocks", True):
...
def restoreBOOTPROTandNVMCTRLREGIONLOCKS():
    ...
    if "M0" in architecture:
        import re
        if re.match("PIC32CM\d{4}JH\d{5}", device):
